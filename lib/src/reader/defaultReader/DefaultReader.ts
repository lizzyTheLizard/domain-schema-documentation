...
import { type DefaultInputNormalizerOptions, defaultInputNormalizer } from './DefaultInputNormalizer'
 ...
 */
export interface DefaultReaderOptions extends DefaultInputNormalizerOptions {
  /**
...
export function defaultReader (inputFolder: string, optionsOrUndefined?: Partial<DefaultReaderOptions>): Reader {
  return async function (): Promise<Model> {
    ...
    const inputNormalizer = defaultInputNormalizer(options)
...
function applyDefaults (options?: Partial<DefaultReaderOptions>): DefaultReaderOptions {
  return {
    ...
    ajvOptions: options?.ajvOptions ?? { allErrors: true, discriminator: true },
