import { run, defaultReader, javaPlugin } from 'domain-schema-documentation'
...
fs.rm(output, { recursive: true, force: true })
  .then(async () => { await run({ reader: defaultReader(input), plugins: [javaPlugin(output, { srcDir: src, ignoreAdditionalFiles: false })] }) })
