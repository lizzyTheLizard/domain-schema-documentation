...
import { type Schema } from '../Reader.ts'
...
describe('InputValidator.Schema', () => {
  ...
  test('schema file with invalid schema', () => {
    ...
    expect(() => { inputValidator.validateSchemaFile(schemaFile) })
      .toThrow(new Error('Invalid schema /Module/Schema.yaml: data must have required property \'enum\', data/type must be equal to one of the allowed values, data/type must be equal to one of the allowed values, data must have required property \'oneOf\', data/type must be equal to one of the allowed values, data must match exactly one schema in oneOf'))
  })
  ...
  test('schema file with missing enum documentation value', () => {
    ...
    expect(() => { inputValidator.validateSchemaFile(schemaFile) })
      .toThrow(new Error('Schema /Module/Schema.yaml has an \'x-enum-description\' but is missing documentation for enum value \'key2\''))
  })
  ...
  test('schema file with additional enum documentation value', () => {
    ...
    expect(() => { inputValidator.validateSchemaFile(schemaFile) })
      .toThrow(new Error('Schema /Module/Schema.yaml has an \'x-enum-description\' for enum value \'key3\' that does not exist'))
  })
  ...
  test('schema file with undefined required', () => {
    ...
    expect(() => { inputValidator.validateSchemaFile(schemaFile) })
      .toThrow(new Error('Schema /Module/Schema.yaml has a required property \'key2\' that is not defined'))
  })
